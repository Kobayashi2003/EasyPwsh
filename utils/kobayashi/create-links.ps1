...
try {
    ...
    foreach ($item in $items) {
        ...
        if (-not $itemname) {
            Write-Host "⚠️ Cannot create link for $item without a name" -ForegroundColor Yellow
            continue
        }
        if (Test-Path "$path\$itemname") {
            Write-Host "⚠️ Link already exists for $item" -ForegroundColor Yellow
            continue
        }

        New-Item -ItemType SymbolicLink -Path "$path" -Name "$itemname" -Target "$item" | Out-Null
        Write-Host "✔️ Linked $itemname -> $item" -ForegroundColor Green
    }
